...
function Get-ConnectionString {
    ...
    if ($script:SessionConfig.Server) {
        ...
        $serverPrompt = "SQL Server name (Press Enter to use saved: $($script:SessionConfig.Server))"
    ...
    if ($script:SessionConfig.Database) {
        ...
        $dbPrompt = "Database name (Press Enter to use saved: $($script:SessionConfig.Database))"
    ...
    if ($script:SessionConfig.AuthMethod) {
        ...
        Write-Host "`nSaved Authentication: $authDefault" -ForegroundColor Gray
        $authPrompt = "Change authentication? Press Enter to use saved (1=Windows, 2=SQL)"
    ...
    if ($authMethod -eq "1") {
        ...
        Write-Host "`n[OK] Using Windows Authentication" -ForegroundColor Green
        $username = ""
    } else {
        # Get username
        if ($script:SessionConfig.Username) {
            Write-Host "Saved Username: $($script:SessionConfig.Username)" -ForegroundColor Gray
            $userPrompt = "Username (Press Enter to use saved: $($script:SessionConfig.Username))"
        ...
        Write-Host "`n[OK] Using SQL Server Authentication" -ForegroundColor Green
    }
...
function Initialize-DABConfig {
    ...
    Write-Host "`n[*] Initializing DAB Configuration..." -ForegroundColor Cyan
...
function Discover-Tables {
    ...
    Write-Host "`n[*] Discovering database tables..." -ForegroundColor Cyan
...
function Add-TablesToConfig {
    ...
    Write-Host "`n[*] Adding tables to configuration..." -ForegroundColor Cyan
    $addedCount = 0
    $skippedCount = 0
    ...
    Write-Host "`n[OK] Configuration Complete!" -ForegroundColor Green
...
function Setup-MCP {
    ...
    $confirm = Read-Host "`nAdd all $($tables.Count) tables to configuration? (Y/N)"
    if ($confirm -ne "Y") {
    ...
    Write-Host "`n[*] Creating Claude Code MCP configuration..." -ForegroundColor Cyan
    $mcpDir = ".\.claude"
...
function Start-MCPServer {
    ...
    Write-Host "`n[*] Starting DAB in MCP mode..." -ForegroundColor Cyan
...
function Test-MCPConnection {
    ...
    try {
        ...
        if ($output) {
            ...
            Write-Host "`nEntities in configuration:" -ForegroundColor Cyan
...
function View-Configuration {
    ...
    if (Test-Path $configPath) {
        Write-Host "`n[DAB Configuration]" -ForegroundColor Cyan
    ...
    if (Test-Path $mcpConfigPath) {
        Write-Host "`n[Claude MCP Configuration]" -ForegroundColor Cyan
        Get-Content $mcpConfigPath | Write-Host
    } else {
        Write-Host "`n[WARN] Claude MCP configuration not found" -ForegroundColor Yellow
    }
