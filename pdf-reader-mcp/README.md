...
For a local or self-hosted streamable HTTP MCP endpoint, Docker runs the
sole-Rust server and only grants it read access to the mounted `./documents`
directory.

```bash
mkdir documents
MCP_API_KEY=replace-with-a-long-random-value docker compose up --build -d
```

The health endpoint is `http://127.0.0.1:3000/mcp/health`. Connect MCP clients
to `http://127.0.0.1:3000/mcp` with `X-API-Key` set to `MCP_API_KEY`; PDFs must
be placed in `./documents`. For reverse-proxy access, set `MCP_ALLOWED_HOSTS` to
the public hostname before starting the stack.
